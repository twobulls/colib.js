import { changeToColor, changeFromColor } from './color';
...
describe('changeToColor', () => {
  ...
  it('can tween ColorHSL to a target color', () => {
    ...
    queue.enqueue(changeToColor(ref, target, 1));
    queue.update(0.25);
    expect(ref).toEqual({ h: 80, s: 0.75, l: 0.25 });
  ...
  it('can tween ColorHSL to a target color with alpha', () => {
    ...
    queue.enqueue(changeToColor(ref, target, 1));
    queue.update(0.25);
    expect(ref).toEqual({ h: 80, s: 0.75, l: 0.25, a: 0.35 });
    queue.update(1);
  ...
  it('can tween ColorHSV to a target color', () => {
    ...
    queue.enqueue(changeToColor(ref, target, 1));
    queue.update(0.25);
    expect(ref).toEqual({ h: 80, s: 0.75, v: 0.25 });
  ...
  it('can tween ColorHSV to a target color with alpha', () => {
    ...
    queue.enqueue(changeToColor(ref, target, 1));
    queue.update(0.25);
    expect(ref).toEqual({ h: 80, s: 0.75, v: 0.25, a: 0.35 });
    queue.update(1);
